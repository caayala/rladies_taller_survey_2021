...
svymean( ~ edad, cas_svy, na.rm = TRUE, deff = TRUE) # error estándar
...
sv2 <- subset(cas24_svy, is.na(esc) == FALSE)

svyby(~ esc, ~ catedad, sv2,  na.rm = TRUE, svymean, covmat=TRUE)

t1 <- svycontrast(svyby(~ esc, ~ catedad, sv2, na.rm = TRUE, svymean, covmat = TRUE), 
                  c( 0, -1, 0, 1))

t1
confint(t1)

## otra alternativa que maneja mejor los missing: usar regresión lineal.
...
# Con este método no se puede calcular el intervalo de confianza, 
# No se debiese usar esta manera:
# confint(svymean( ~ fapoyo, cep_svy, na.rm = TRUE))

# method = c("logit", "likelihood", "asin", "beta", "mean","xlogit")
# proporciones 
svyciprop( ~I(apoyo==1), cep_svy, method = 'logit') # método por defecto
...
### 
svymean( ~ fmanif, cep_svy, na.rm = TRUE, deff = TRUE)
